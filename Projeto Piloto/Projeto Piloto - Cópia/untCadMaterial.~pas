...
type
  TfrmCadMaterial = class(TfrmCadPadrao)
    ...
    qryMaterialTIPOMATERIAL: TSmallintField;
...
procedure TfrmCadMaterial.qryMaterialNewRecord(DataSet: TDataSet);
  ...
  if (qryMaterial.FieldByName('codigomaterial').IsNull) then begin
      sqlAI.SQL.Text := 'select max(codigomaterialhistorico) as codigomaterialhistorico from MATERIAL';
      sqlAI.ExecQuery;
  try
   qryMaterial.FieldByName('codigomaterial').AsInteger :=
   sqlAI.FieldByName('codigomaterialhistorico').AsInteger + 1;
    finally
      sqlAI.Close;
    end;
...
//Mostra a mensagem do erro ocorrido.
      Application.MessageBox(PChar(strMsg), PChar(strTitulo), MB_ICONEXCLAMATION +
      MB_OK);
      Action := daAbort;
  end;
