...
procedure TfrmCadGrupo.qryGrupoNewRecord(DataSet: TDataSet);
  ...
  inherited;
    if (qryGrupo.FieldByName('CODIGOGRUPO').IsNull) then begin
         ...
         sqlAI.ExecQuery;
      try
         qryGrupo.FieldByName('CODIGOGRUPO').AsInteger :=
         sqlAI.FieldByName('CODIGOGRUPO').AsInteger + 1;
      finally
         sqlAI.Close;
      end;
         qryGrupo.FieldByName('MEDICAMENTO').FocusControl;
    end;
  end;
...
//Verifica se todos os campos foram preenchidos.
    if Pos(UpperCase('*** null ***'), UpperCase(E.Message)) <> 0 then begin
      ...
      end else if Pos(UpperCase('MEDICAMENTO'), UpperCase(E.Message)) <> 0 then begin
    ...
    if (btnFiltro.Caption = 'Desabilitar') then begin
      if (rbCodigoFiltro.Checked) Then
        ...
        if (rbDescricaoFiltro.Checked) Then
            strSQL := 'select * from GRUPO where upper(MEDICAMENTO) >= ' +QuotedStr(UpperCase(edtDe.Text)) +' and upper(MEDICAMENTO) <= ' +QuotedStr(UpperCase(edtAte.Text));
        end else
            strSQL := 'select * from GRUPO';
              if (rgOrdem.ItemIndex = 0) then
                  strSQL := strSQL + ' order by CODIGOGRUPO'
              else
               strSQL := strSQL + ' order by MEDICAMENTO';
