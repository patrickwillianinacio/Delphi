...
type
  ...
  public
   ...
   Pagina: Integer;
   QrLabel: array[1..20] of TQrLabel;
   QrDbImage: array[1..20] of TqrDbImage;
   QrDbText: array[1..20] of TQrDbtext;
   Desc,Campo: array[1..20] of String;
   Largura: array[1..20] of Integer;
   ...
   intCampo := 0;
    for intCampoTabela:= 0 To (qryTabela.FieldCount - 1) do
      if (qryTabela.Fields[intCampoTabela].Visible) then begin
         Campo[intCampo + 1]:= qryTabela.Fields[intCampoTabela].FieldName;
         intCampo := intCampo + 1;
      end;
//Matriz Com Descrição das Colunas do Relatório
      intCampo := 0;
         for intCampoTabela:= 0 To (qryTabela.FieldCount - 1) do
            if (qryTabela.Fields[intCampoTabela].Visible) then begin
               Desc[intCampo + 1]:= qryTabela.Fields[intCampoTabela].DisplayName;
               intCampo := intCampo + 1;
            end;
//Matriz Com a Largura das Colunas
         ...
         for intCampoTabela := 0 To (qryTabela.FieldCount - 1) do
            if (qryTabela.Fields[intCampoTabela].Visible) then begin
              if (Length(Desc[intCampo + 1]) > qryTabela.Fields[intCampoTabela].DisplayWidth) then
                  Largura[intCampo + 1]:= Length(Desc[intCampo + 1])
              else
               Largura[intCampo + 1]:= qryTabela.Fields[intCampoTabela].DisplayWidth;
               intCampo := intCampo + 1;
              end;
    ...
    if (ltbCampo.Items.Strings[ltbCampo.ItemIndex] = 'Indisponível') then
      exit;
       if (Pagina + Largura[PosCampo(ltbCampo.Items.Strings[ltbCampo.ItemIndex])] >125) then
            Application.MessageBox('Campo Supera o Tamanho da Pagina', 'Aviso', MB_ICONINFORMATION + MB_OK)
       else begin
         Pagina := Pagina + Largura[PosCampo(ltbCampo.Items.Strings[ltbCampo.ItemIndex])];
         ltbCampoSelecionado.Items.Add(ltbCampo.Items.Strings[ltbCampo.ItemIndex]);
