...
/// alterado
      ...
      r.free;
         ...
         begin if (trim(Host.Text) <> '192.168.2.97') Then Begin
...
/// alterado
         ...
         Showmessage('entrou    8 else');


         ////////
                 begin if (cont > 1) Then Begin
                 Showmessage('entrou if cont   > 1');
                 Showmessage('cont'+IntToStr(cont));

                           S_Server.Active := True;
                           Servir.Enabled := False;
                           S_Cliente.Active := True;
                           Servir.Caption := 'Servidor Ligado';
                           Host.Enabled := False;
...
/// alterado
                     ...
                     Showmessage('entrou    9');
//// alterado
                  begin if (trim(Host.Text) <> '192.168.2.97') Then Begin

                  Showmessage('entrou    10');
                           Status.Lines.Add('Servidor ::> Adicione um Apelido!');
                           Servir.Caption := 'Iniciar Servidor';
                           Host.Enabled := True;
                           Conectar.Enabled := True;
                  end else begin
