...
uses
  ...
  Dialogs, StdCtrls, Sockets, ScktComp, ExtCtrls,IdBaseComponent, IdComponent, IdIPWatch;
...
procedure TChatFal.FormCreate(Sender: TObject);
  begin
   ...
   BitmapImage.LoadFromFile('C:\Users\root\Desktop\Tarefas\Testes\SnaipesChat\icons\fundo3.bmp');
   Quadro.brush.Bitmap:= TBitmap.Create;
   Quadro.brush.Bitmap.LoadFromFile('C:\Users\root\Desktop\Tarefas\Testes\SnaipesChat\icons\fundo3.bmp');
   BitmapImage := TBitmap.Create;
   BitmapImage.LoadFromFile('C:\Users\root\Desktop\Tarefas\Testes\SnaipesChat\icons\fundo3.bmp');
   Status.brush.Bitmap:= TBitmap.Create;
   Status.brush.Bitmap.LoadFromFile('C:\Users\root\Desktop\Tarefas\Testes\SnaipesChat\icons\fundo3.bmp');
   C_Texto.Text:= '';
   Apelido.Text:= '';
...
procedure TChatFal.ConectarClick(Sender: TObject);
  begin if (trim(Apelido.Text )<> '') Then Begin
    begin if (S_Cliente.Active) then begin
             ...
             Servir.Enabled := true;
                     ...
                     Servir.Enabled := False;
                     Host.Enabled := True;
                     Conectar.Enabled := True;
              end;
          end;
  end else begin
                  Status.Lines.Add('Conectar ::> Adicione o Servidor e Apelido ');
                  S_Cliente.Active := False;
                  Conectar.Caption := 'Conectar';
      end;
  end;
  ...
  if texto[0] = 'NICK' then {Verifica se a mensagem eh de entrada} begin
      WITH S_Server.Socket DO BEGIN {Se a msg for de entrada avisa a todos os clientes quem entrou }
        ...
        WITH S_Server.Socket DO BEGIN {Se nao for de entrada, então eh msg normal, no caso passa para todos a msg}
          begin if (trim(texto[0]) <> '') Then Begin
            FOR Index := 0 TO ActiveConnections-1 DO BEGIN
